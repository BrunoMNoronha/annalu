...
| HIST-ROUND-002 | Exibir charada e capturar resposta | Núcleo do jogo | 🔎 PARCIAL — **leitura/enunciado no backend entregue**: `getRoundState` devolve os `SessionChallenge` persistidos (ordenados por `position`) com `Riddle.prompt`, sem expor palavra-alvo/respostas aceitas (minimização de dados). **Falta:** digitação/salvamento da resposta (`PlayerAnswer`) e a UI. Ver `src/modules/round` (`getRoundState`) | HIST-ROUND-001 | Alta |
