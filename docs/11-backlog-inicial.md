...
| HIST-ROUND-002 | Exibir charada e capturar resposta | Núcleo do jogo | 🔎 PARCIAL — **backend entregue**: (a) **enunciado** — `getRoundState` devolve os `SessionChallenge` persistidos (ordenados por `position`) com `Riddle.prompt`, sem expor palavra-alvo/respostas aceitas; (b) **salvamento de rascunho** — `saveAnswerDraft` cria/atualiza um `PlayerAnswer` `DRAFT` por desafio, texto **literal** (sem correção/normalização), editável só com a rodada `IN_PROGRESS` no prazo (coordenação save × expiração sob lock da sessão); (c) **restauração** do rascunho no `getRoundState` (`answer` = `null`/`""`/texto). **Falta:** UI, imagem e submissão. Ver `src/modules/round` (`getRoundState`, `saveAnswerDraft`) | HIST-ROUND-001 | Alta |
| HIST-ROUND-003 | Temporizador da sessão | Regra de tempo | 🔎 PARCIAL — **backend de expiração entregue**: `expireRoundIfDue` transiciona `IN_PROGRESS → EXPIRED` de forma **atômica e idempotente** quando `now >= expiresAt` (servidor é a autoridade — RN-TMP-002), com `endedAt = expiresAt`; a consulta `getRoundState` **dispara a expiração sob demanda** e retorna o **tempo restante calculado no servidor** (`remainingMilliseconds`). Sem worker/cron. **Falta** o contador visual/UI. Ver `src/modules/round` (`expireRoundIfDue`, `getRoundState`) | HIST-CFG-002; DEC-009 ✅ | Alta |
| HIST-ROUND-004 | Encerrar/finalizar rodada | Conclusão do fluxo | Rodada encerra e confirma envio | HIST-ROUND-002 | Alta |
| HIST-ROUND-005 | Pular charada | Flexibilidade | Pular conforme regra (**CONFIRMADO** — DEC-008 / `RN-PUL-001..003`: estado `pulado`, zero ponto, sem troca). **Não** implementado nesta fatia | DEC-008 ✅ | Baixa |
